...
#define NOTIFY_a_OR_EMPTY(str)		(!str ||! strlen(str))
...
private:
    ...
    auto set_content(const char* format, va_list args) { vsnprintf(this->content, sizeof(this->content), format, args); }

public:

    auto set_title(const char* format, ...) -> void { NOTIFY_FORMAT(this->set_title, format); }

    auto set_content(const char* format, ...) -> void { NOTIFY_FORMAT(this->set_content, format); }

    auto set_type(const ImGuiToastType& type) -> void { IM_ASSERT(type < ImGuiToastType_COUNT); this->type = type; };
    ...
    NOTIFY_INLINE VOID RenderNotifications()
    {
        ...
        for (auto i = 0; i < notifications.size(); i++)
        {
            ...
            // Here we render the toast content
            {
                ...
                else if (!NOTIFY_a_OR_EMPTY(default_title))
                {
                    if (!NOTIFY_a_OR_EMPTY(icon))
                        SameLine();
                ...
                if (was_title_rendered && !NOTIFY_a_OR_EMPTY(content))
                {
                    SetCursorPosY(GetCursorPosY() + 5.f); // Must be a better way to do this!!!!
                }

                // If a content is set
                if (!NOTIFY_a_OR_EMPTY(content))
                {
